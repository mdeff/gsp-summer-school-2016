...
There exist two paradigms to model the underlying structure of datasets using graphs: 1) constructing a
\textit{feature graph}, where nodes represent features (e.g. image features extracted from patches in a picture) and samples are graph
signals, or 2) a \textit{data graph}, where nodes represent samples (e.g. weather data in a sensor network) and graph
signals are functions of the data. In the first case the data is transformed using
GSP tools. An example of this is non-local denoising of an image. Inpainting is an example of the second cas where graphs are often used to regularize
...
\paragraph{Constructed data graph.}
